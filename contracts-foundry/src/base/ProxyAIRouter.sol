...
abstract contract ProxyAIRouter {
    ...
    function setControllerVault(address _controllerVault) external {
        controllerVault = _controllerVault;
    }

    function setControllerChainSelector(
        uint64 _controllerChainSelector
    ) external {
        controllerChainSelector = _controllerChainSelector;
    }
    ...
    function depositToFeeTank(address admin, uint256 amount) external {
        feeTank[admin] += amount;
    ...
    function _generateKey(
        ...
        expectedIdempotencyKey = keccak256(
