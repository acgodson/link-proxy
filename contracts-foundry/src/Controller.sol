...
contract Controller is CCIPReceiver {
    ...
    enum OperationType { Low, Medium, High }
    enum FunctionType { GenerateKey, SubmitReceipt }
    ...
    event KeyGenerated(bytes32 indexed idempotencyKey, address proxy, uint64 sourceChainSelector, address sourceAddress);
    event ReceiptSubmitted(bytes32 indexed idempotencyKey, address token, uint256 usedTokens);

    constructor(address router) CCIPReceiver(router) {}

    function _ccipReceive(Client.Any2EVMMessage memory message) internal override {
        (FunctionType functionType, bytes memory functionPayload) = abi.decode(message.data, (FunctionType, bytes));

        if (functionType == FunctionType.GenerateKey) {
            _handleGenerateKey(functionPayload, message.sourceChainSelector, abi.decode(message.sender, (address)));
        } else {
            revert("Unsupported function");
        }
    }

    function _handleGenerateKey(bytes memory functionPayload, uint64 sourceChainSelector, address sourceAddress) internal {
        (address proxy, bytes32 requestHash, OperationType predictedTokenUsage, uint256 fixedNonce) = abi.decode(
            functionPayload,
    ...
    function getIdempotencyData(bytes32 key) public view returns (
        address proxy,
        OperationType predictedTokenUsage,
        bool processed,
        uint256 expirationTime
